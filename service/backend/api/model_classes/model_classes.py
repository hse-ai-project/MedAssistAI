...
class HeartDataImputer:
    ...
    def fit(self, data: pd.DataFrame):
        ...
        Также обучает StandardScaler и преобразовывает данные
    ...
    def transform(self, patient_data: dict) -> dict:
        ...
        с возможными пропущенными значениями
        ...
        # Scaler
        filled_data[self.features["numeric"]] = self.scaler.transform(
            filled_data[self.features["numeric"]]
        )
...
class HeartBasedPredictor:
    ...
    def __init__(self):
        ...
        self.log_params = {"random_state": 42, "max_iter": 1000}
    ...
    def fit(self, X: Dict[str, List[Any]],
        ...
        self.imputer = self.imputer.fit(
            X
        )  # Сначала обучаем новый импутер на новых данных
        X_processed = self.preprocess(
            df
        )  # Преобразовываем данные на новом препроцессинге
...
class CardioTrainBasePredictor:
    ...
    def preprocessing_fit(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Функция, устанавливающая средние значения обучающего датафрейма в
        качестве значений для заполнения пропусков на предикте
    ...
    def preprocessing_transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        В виду того, что данная модель является базовой, в
        данном варианте присутствуют только заполнения пропусков на случай,
    ...
    def fit(self, X: Dict[str, List[Any]], y: List[int]):
        """Обучение модели"""
        self.preprocessing_fit(
            pd.DataFrame(X)[self.fixed_features]
        )  # Получение готовых пропусков для заполнения в дальнейшем
...
class PredictorComposer:
    ...
    def __init__(
        self,
        heart_based_predictor: HeartBasedPredictor,
    ...
    def fit_splitted(
        self,
        X1: Dict[str, List[Any]],
        y1: np.ndarray,
        X2: Dict[str, List[Any]],
        y2: np.ndarray,
    ) -> "PredictorComposer":
